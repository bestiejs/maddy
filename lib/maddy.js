...
(function (root, Maddy) {
  ...
  // **keys** returns a lexicographically-sorted array containing an object's member names.
  exports["keys"] = keys;
  ...
  // **fold** reduces an object to a single value by invoking the callback on each member. `memo`
  // specifies the initial state of the reduction; each successive nth invocation of callback should
  // subsequently return the value of `memo` to be used in the (n+1)th invocation. Aliased as
  // **inject** and **reduce**.
  ...
  // **groupBy** groups an object's members using the criteria provided by the callback.
